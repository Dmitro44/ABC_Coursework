...
Ядро #emph("Linux") само по себе не является готовой к использованию операционной системой. Оно распространяется в составе дистрибутивов — комплексных программных сборок, включающих ядро, системные утилиты, библиотеки, графический сервер и прикладное программное обеспечение. Многообразие дистрибутивов обусловлено различиями в системах управления пакетами, моделях выпуска обновлений и целевом назначении. Основные семейства дистрибутивов включают:

+ #[На базе #emph("Debian"): характеризуются использованием пакетного менеджера #emph("APT") и формата #emph(".deb"). Примерами являются сам #emph("Debian"), ориентированный на стабильность, и #emph("Ubuntu"), нацеленный на удобство использования на настольных компьютерах и серверах.]

+ #[На базе #emph("Red Hat"): используют систему управления пакетами #emph("RPM") и менеджеры #emph("DNF") или #emph("YUM"). К этому семейству относятся #emph("Fedora"), служащая инновационной площадкой для технологий, включаемых в коммерческий дистрибутив #emph("Red Hat Enterprise Linux") (#emph("RHEL")).]

+ #[На базе #emph("Arch"): отличаются применением модели непрерывных обновлений (#emph("rolling release")) и пакетного менеджера #emph("Pacman"). #emph("Arch Linux") предоставляет минималистичную базовую систему, предоставляя пользователю полный контроль над её конфигурацией.]
