...
Метрики #emph("Intel") без #emph("Hyper-Threading") показывают существенный рост #emph("IPC") для #emph("P") ядрер, подтверждая эффективность монопольного использования ресурсов ядра. #emph("E") ядра демонстрируют #emph("IPC") около единицы, что отражает архитектурные ограничения микроархитектуры #emph("Gracemont").  Высокий процент промахов кэша на обоих типах ядер указывает на подсистему памяти как ключевой ограничивающий фактор.
