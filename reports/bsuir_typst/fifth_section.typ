...
Для сбора данных о производительности использовался набор стандартных утилит. Основные аппаратные метрики собирались с помощью инструмента #emph("perf stat") по следующим событиям: #emph("cycles"), #emph("instructions"), #emph("cache-references"), #emph("cache-misses"), #emph("branch-instructions") и #emph("branch-misses"). Эти данные позволили в дальнейшем рассчитать такие показатели, как #emph("IPC") (инструкций за такт) и процент промахов. Одновременно с этим, утилита #emph("mpstat") использовалась для фонового мониторинга и сбора данных о нагрузке на каждое логическое ядро процессора, что позволяло верифицировать корректность распределения нагрузки в ходе теста. Детальные графики сравнения производительности представлены в приложении Д, а нагрузка на ядра процессоров в ходе тестирования отображена в приложении Е.
...
#figure(
  ...
  table(
    ...
    align: center + horizon,
    [Метрика], [Одно ядро], [4 ядра], [8 ядер],
    [IPC], [8.01 / -], [3.43 / -], [4.48 / 0.81],
    [Промахи кэша (%)], [75.65 / -], [44.40 / -], [49.53 / 62.25],
    [Промахи ветвлений (%)], [0.10 / -], [0.14 / -], [0.13 / 0.16],
  ),
) <smt_on_intel_perf>
...
#figure(
  ...
  stp.longtable(
    ...
    table.header(
      ...
      align(center)[#emph("Intel UHD Graphics Xe G4")],
      table.hline(stroke:black),
    ),
    [32], [0.0111], [0.032654],
    [64], [0.009496], [0.037497],
    [128], [0.01026], [0.034422],
    [256], [0.013364], [0.039265],
    [512], [0.016054], [0.054421],
