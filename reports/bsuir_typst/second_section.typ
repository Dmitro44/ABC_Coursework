...
Для проведения исследования в качестве дистрибутива операционной системы #emph("GNU/Linux") был выбран #emph("Arch Linux") @ArchLinuxCons. Этот выбор обусловлен рядом ключевых преимуществ, которые делают его оптимальной платформой для проведения точного и воспроизводимого сравнительного анализа производительности.
...
Компиляция программы будет производиться с помощью компилятора #emph("GCC") последней версии, доступной в дистрибутиве. Для достижения максимальной производительности компиляция будет выполняться с флагом #emph("-march=native"), который активирует высокий уровень оптимизации. Флаг #box[#emph("-march=native")] заставляет компилятор генерировать код, оптимизированный под специфический набор инструкций конкретного процессора, на котором производится сборка. Это позволит в полной мере задействовать архитектурные преимущества каждого из тестируемых #emph("CPU").
...
Для получения более глубоких данных о производительности, помимо простого измерения времени выполнения, планируется использование утилиты #emph("perf") @perfAbout. Этот инструмент позволяет собирать данные с аппаратных счетчиков производительности процессора, такие как количество выполненных инструкций, промахи кэша и неверно предсказанные переходы. Анализ этих метрик даст возможность сделать более обоснованные выводы о причинах наблюдаемых различий в производительности.

Помимо #emph("perf"), для мониторинга и сбора данных о нагрузке на каждое логическое ядро процессора будет использоваться утилита #emph("mpstat") @mpstatAbout. Эта утилита, входящая в состав пакета #emph("sysstat"), предназначена для сбора и отображения статистики по каждому доступному процессору. #emph("mpstat") предоставляет детализированную информацию об использовании процессорного времени, разделяя его на несколько категорий: время, затраченное на выполнение пользовательских процессов (#emph("%usr")), системных вызовов (#emph("%sys")), ожидание ввода-вывода (#emph("%iowait")) и время простоя (#emph("%idle")). В рамках данного исследования #emph("mpstat") будет использоваться для верификации корректности распределения нагрузки между ядрами в ходе тестирования и для получения полной картины использования ресурсов #emph("CPU"), что особенно важно при анализе работы планировщика на гибридной архитектуре.
