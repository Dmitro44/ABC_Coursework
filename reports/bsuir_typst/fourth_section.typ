...
+ #[Генерация входных данных. Создается синтетический набор комплексных чисел размером $2^n$, где $n$ варьируется в диапазоне от 5 до 25, что позволяет исследовать производительность на различных масштабах данных от нескольких килобайт до нескольких мегабайт.]
...
+ #[Тестирование на интегрированных графических ускорителях (#emph("iGPU")). Программа включает отдельный модуль для выполнения алгоритма БПФ на #emph("iGPU") с использованием технологии #emph("OpenCL"), что позволяет сравнивать производительность различных интегрированных графических ускорителей в вычислительно интенсивных задачах.]
...
Блок-схема алгоритма программного средства представлена в приложении Г и состоит из четырёх взаимосвязанных частей: главного алгоритма, алгоритмов однопоточного и многопоточного БПФ для CPU, а также алгоритма для iGPU.

Главный алгоритм определяет общую логику работы программы. После запуска и получения входных данных, в зависимости от параметров, вызываются подпрограммы для тестирования на CPU (однопоточный или многопоточный режим). Затем управление передаётся подпрограмме для выполнения БПФ на iGPU. По завершении всех вычислений результаты сохраняются в файлы формата #emph("CSV").
...
Наконец, подпрограмма для iGPU, основанная на технологии OpenCL, описывает процесс вычислений на графическом ускорителе. Она включает этапы инициализации платформы, компиляции ядра, создания буферов и копирования данных с хоста на устройство. Основные вычисления, включая бит-реверсивную перестановку и итеративные стадии «бабочек», выполняются параллельно на iGPU путем запуска соответствующих ядер. По завершении вычислений результаты копируются обратно на хост для сохранения.
